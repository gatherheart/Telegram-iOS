...
private func requestStats(postbox: Postbox, network: Network, datacenterId: Int32, peerId: PeerId, dark: Bool = false) -> Signal<ChannelStats?, NoError> {
    return postbox.transaction { transaction -> Peer? in
        ...
        if network.datacenterId != datacenterId {
            signal = network.download(datacenterId: Int(datacenterId), isMedia: false, tag: nil, hint: "ChannelStatistics.requestStats(datacenterId \(datacenterId), peerId \(peerId), dark \(dark)")
...
private func requestGraph(network: Network, datacenterId: Int32, token: String, x: Int64? = nil) -> Signal<ChannelStatsGraph?, NoError> {
    ...
    if network.datacenterId != datacenterId {
        signal = network.download(datacenterId: Int(datacenterId), isMedia: false, tag: nil, hint: "ChannelStatistics.requestGraph(datacenterId \(datacenterId), token \(token), x \(x)")
        |> castError(MTRpcError.self)
        |> mapToSignal { worker in
